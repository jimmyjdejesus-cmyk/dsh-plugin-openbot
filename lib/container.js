...
export class OpenBotContainerManager {
  ...
  constructor(options = {}) {
    ...
    // Logger instance for diagnostic reporting
    this.logger = options.logger || console;
    // Primary Docker image to run (prefers local dsh-openbot-sandbox if built)
    this.imageName = options.imageName || process.env.OPENBOT_IMAGE || 'dsh-openbot-sandbox:latest';
    // Upstream fallback image name
    this.fallbackImage = 'ghcr.io/copilotkit/openbot-sandbox:latest';
    // Active running state tracker
  ...
  async ensureRunning() {
    ...
    // Helper function to execute docker run command with mapped ports and workspace mount
    const attemptLaunch = async (image) => {
      const runCmd = [
        'docker run -d',
    ...
    try {
      // Primary attempt with local/configured image
      await attemptLaunch(this.imageName);
      this.isRunning = true;
      this.logger.info?.(`OpenBot container launched successfully with ${this.imageName} on ports ${this.apiPort}, ${this.vncPort}`);
      return { mode: 'docker', running: true, image: this.imageName };
    } catch (err) {
      this.logger.warn?.(`Could not launch primary image ${this.imageName}: ${err.message}. Checking fallback...`);
      
      // Secondary attempt with upstream fallback image if different
      if (this.imageName !== this.fallbackImage) {
        try {
          await attemptLaunch(this.fallbackImage);
          this.isRunning = true;
          this.logger.info?.(`OpenBot container launched successfully with fallback image ${this.fallbackImage}`);
          return { mode: 'docker', running: true, image: this.fallbackImage };
        } catch (fallbackErr) {
          this.logger.warn?.(`Fallback image ${this.fallbackImage} also failed: ${fallbackErr.message}`);
        }
      }
      
      // Graceful fallback to isolated local execution
      this.logger.warn?.('Docker container launch failed; operating in local jailed sandbox mode.');
      return { mode: 'fallback', running: false };
    }
  }
