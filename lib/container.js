 ...
 * Orchestrates container boot, health checks, command execution, and workspace mounting.
 */
...
export class OpenBotContainerManager {
   ...
   * @param {string} options.containerName - Name of the Docker container (default: 'dsh-openbot-sandbox')
   * @param {string} options.workspaceDir - Host directory mounted to /sandbox/workspace in the container
   * @param {number} options.apiPort - Port mapped for OpenBot / CDP API (default: 8080)
   * @param {number} options.vncPort - Port mapped for noVNC / Live Screencast (default: 8081)
  ...
  async getStatus() {
    try {
      ...
      return { exists: true, running: this.isRunning, status };
    } catch (err) {
      this.isRunning = false;
