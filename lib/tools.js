...
export function registerOpenBotTools(ctx, containerManager, securityGateway) {
  ...
  registerTool(ctx, {
    ...
    async execute(params = {}) {
      return await securityGateway.executeGuarded('openbot_browser_snapshot', params, async () => {
        ...
        if (res.exitCode !== 0) {
          throw new Error(`Browser snapshot failed (exit code ${res.exitCode}): ${res.stderr || res.stdout}`);
        }
  ...
  registerTool(ctx, {
    ...
    async execute(params = {}) {
      return await securityGateway.executeGuarded('openbot_browser_screenshot', params, async () => {
        ...
        const rawFilename = params.saveName || `screenshot_${Date.now()}.png`;
        const filename = path.basename(rawFilename);
        const hostFilePath = path.join(containerManager.workspaceDir, filename);

        // In container, workingDir is /sandbox/workspace; in fallback, cwd is host workspaceDir.
        // Executing with just the sanitized filename ensures it lands in the shared mounted workspace.
        const res = await containerManager.exec(`npx -y agent-browser screenshot "${filename}"`);
        return {
          status: res.exitCode === 0 ? 'captured' : 'failed',
          filename,
          workspacePath: hostFilePath,
          output: res.stdout || res.stderr
        };
      });
    }
  ...
  // ============================================================================
  // 4. HUMAN-IN-THE-LOOP ("TAKE THE WHEEL") INTERACTIVE CONTROLS
  // ============================================================================

  // 4.1 Take the Wheel
  registerTool(ctx, {
    name: 'openbot_take_wheel',
    description: 'Engage Human-in-the-Loop "Take the Wheel" mode. Suspends automated browser actions while the user manually interacts (e.g. solving CAPTCHAs, 2FA, logins).',
    parameters: {
      type: 'object',
      properties: {
        reason: { type: 'string', description: 'Optional explanation of why manual intervention is needed' }
      }
    },
    async execute(params = {}) {
      ...
      return {
        mode: 'human_driving',
        active: true,
        message: '🚗 Human user has taken the wheel. Agent browser actions are suspended.',
        reason: params.reason || 'User manual takeover active'
      };
    }
  });

  // 4.2 Release the Wheel
  registerTool(ctx, {
    name: 'openbot_release_wheel',
    description: 'Release Human-in-the-Loop "Take the Wheel" mode and resume autonomous agent control over browser actions.',
    parameters: {
      type: 'object',
      properties: {}
    },
    async execute() {
      ...
      return {
        ...
        active: false,
        message: '🤖 Human released the wheel. Agent has resumed autonomous control.'
      };
    }
  });
}
