...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      if (!ctx.slots) return;
      try {
        const renderComponent = () => React ? React.createElement(OpenBotDashboard, { ctx }) : null;

        // Direct registration for workspace tab and settings item
        ...
        // Also register for DSH web-ui plugin slots via inject if available
        if (typeof ctx.slots.inject === "function") {
          ctx.slots.inject("web-ui.plugin.item", () => {
            if (typeof ctx.slots.register === "function") {
              return ctx.slots.register({
                name: "web-ui.plugin.item",
                id: "openbot-sandbox-ui",
                title: "OpenBot Sandbox",
                order: 125
              }, renderComponent);
            }
