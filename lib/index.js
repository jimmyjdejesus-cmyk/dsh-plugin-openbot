...
export async function apply(ctx, config = {}) {
  ...
  // 7. Start embedded live dashboard server on port 8085 (if enabled)
  let dashboardServer = null;
  if (config.dashboard !== false) {
    startDashboardServer({
      containerManager,
      securityGateway,
      port: config.dashboardPort || 8085,
      logger: ctx.logger
    }).then(srv => {
      dashboardServer = srv;
    }).catch(err => {
      ctx.logger?.warn?.('Failed to start OpenBot dashboard server:', err.message);
    });
  }

  // 8. Graceful cleanup on DSH shutdown or plugin disposal
  ctx.on('dispose', async () => {
    ctx.logger?.info?.('DSH shutting down, stopping OpenBot container & dashboard...');
    if (dashboardServer) {
